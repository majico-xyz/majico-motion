...
import { resolveStudioChromeMotion, } from "./majico-motion-presets.js";
import { buildMotionPreferencePatch, hydrateMotionPreferenceState, isMotionPreference, } from "./motion-preference-persist.js";
 ...
 */
export function useStudioMotion(projectId) {
    const [preference, setPreferenceState] = useState("standard");
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!res.ok)
                    throw new Error("Could not load project motion settings");
    ...
    const { resolved } = useMemo(() => resolveStudioChromeMotion(motionOptions), [motionOptions]);
    const cssVars = useMemo(() => resolvedMotionToCssVars(resolved), [resolved]);
    const setPreference = useCallback(async (next) => {
        if (!projectId || !isMotionPreference(next))
            return false;
        setBusy(true);
        setError(null);
        ...
        try {
            ...
            const loadData = loadRes.ok
                ? (await loadRes.json())
                : {};
            const hydrated = hydrateMotionPreferenceState(loadData.design_tokens_json);
