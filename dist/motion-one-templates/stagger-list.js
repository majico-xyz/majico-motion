...
function cubicBezierToMotionOneEasing(easing) {
    const match = /cubic-bezier\(\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)\s*,\s*([\d.]+)\s*\)/i.exec(easing);
    if (!match)
        return `"ease-out"`;
 ...
 */
export function staggerListTemplate(input) {
    const durationSec = parseDurationMs(input.resolved.durationFast) / 1000 || 0.001;
    const staggerSec = parseDurationMs(input.resolved.staggerSibling) / 1000 || 0.001;
